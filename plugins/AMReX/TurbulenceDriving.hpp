...
auto calculate_dispersion(amrex::MultiFab &state)
...
public:
  ...
  explicit turbulentDriving(
      const std::map<std::string, std::string> &turb_params) {
    tg.init_driving(turb_params);
  }

  auto applyDriving(
      amrex::MultiFab &state, const amrex::Real time, const amrex::Real dt_in,
    ...
    for (amrex::MFIter mf(state); mf.isValid(); ++mf) {
      ...
      amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j,
        ...
        const amrex::Real rho =
            data(i, j, k, HydroSystem<problem_t>::density_index);

        amrex::Real dE = 0;

        for (int m = 0; m < AMREX_SPACEDIM; m++) {
          const amrex::Real vel =
              data(i, j, k, HydroSystem<problem_t>::x1Momentum_index + m) / rho;
...
// Function to calculate the mass weighted velocity dispersion in the
...
auto calculate_dispersion(amrex::MultiFab &state)
    ...
    reduce_op.eval(
        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int k)
          ...
          const amrex::Real rho =
              data(i, j, k, HydroSystem<problem_t>::density_index);
          const amrex::Real px =
              data(i, j, k, HydroSystem<problem_t>::x1Momentum_index);
          const amrex::Real py =
              data(i, j, k, HydroSystem<problem_t>::x2Momentum_index);
          const amrex::Real pz =
              data(i, j, k, HydroSystem<problem_t>::x3Momentum_index);
  ...
  auto [sum_rho, sum_px, sum_py, sum_pz, sum_pvx, sum_pvy, sum_pvz] =
      reduce_data.value();

  amrex::GpuArray<amrex::Real, 7> reduce_vec = {
      sum_rho, sum_px, sum_py, sum_pz, sum_pvx, sum_pvy, sum_pvz};
  ...
  const amrex::Real dispx =
      std::sqrt(std::max(0.0, (total_pvx / total_rho) - (v_avg_x * v_avg_x)));
  const amrex::Real dispy =
      std::sqrt(std::max(0.0, (total_pvy / total_rho) - (v_avg_y * v_avg_y)));
  const amrex::Real dispz =
      std::sqrt(std::max(0.0, (total_pvz / total_rho) - (v_avg_z * v_avg_z)));
